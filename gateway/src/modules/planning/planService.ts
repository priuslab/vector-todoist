...
import { planPreviewBodySchema, planPreviewSchema, type PlanPreviewBody, type PlanPreview } from './planSchemas.js';
...
const publicTask = (task: TaskRecord): TaskRecord => { const { user: _user, ...safe } = task; return safe as TaskRecord; };
const publicIdea = <T extends Record<string, unknown>>(idea: T): T => { const { user: _user, ...safe } = idea; return safe as T; };
...
export function createPlanService(deps: {
  ...
  async function today(user: VerifiedUser, date: string, timezone: string) {
    ...
    return { date, timezone, tasks: tasks.map(publicTask), blocks: tasks.map(publicTask), warnings: [] };
  }
  return {
    preview,
    apply,
    today,
    async inbox(user) { return { ideas: (await ideaRepository.list(user)).filter((idea) => idea.status === 'backlog').map(publicIdea), tasks: (await taskRepository.list(user)).filter((task) => task.status === 'inbox').map(publicTask) }; },
