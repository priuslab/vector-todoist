...
export function createAdaptationService(repository: AdaptationRepository, now = () => new Date()): AdaptationService {
  ...
  return {
    ...
    async schedulerAdjustments(user) { return (await this.accepted(user)).map((s) => ({ ...(typeof s.adjustment.category === 'string' ? { category: s.adjustment.category } : {}), ...(typeof s.adjustment.energy === 'string' ? { energy: s.adjustment.energy } : {}), multiplier: clamp(Number(s.adjustment.multiplier), .75, 1.75) })); },
  };
}
