...
const validSessions = (sessions: ExecutionSession[]) => sessions.filter((s) => s.status === 'finished' && Number.isFinite(Number(s.plannedMinutes)) && Number(s.plannedMinutes) > 0 && Number.isFinite(Number(s.actualMinutes)) && Number(s.actualMinutes) > 0 && Boolean(s.finishedAt) && Number.isFinite(Date.parse(String(s.finishedAt)))).slice(-100);
...
export function createAdaptationService(repository: AdaptationRepository, now = () => new Date()): AdaptationService {
  ...
  return {
    ...
    async accept(user, id) { const current = await read(user); if (!current.consent) throw new RepositoryError('NOT_FOUND'); const item = current.suggestions.find((s) => s.id === id && s.status === 'proposed'); if (!item) throw new RepositoryError('NOT_FOUND'); const updated = current.suggestions.map((s) => s.id === id ? { ...s, status: 'accepted' as const } : s); await persist(user, current.consent, current.metrics, updated); return updated.find((s) => s.id === id)!; },
    async reject(user, id) { const current = await read(user); if (!current.consent) throw new RepositoryError('NOT_FOUND'); const item = current.suggestions.find((s) => s.id === id && s.status === 'proposed'); if (!item) throw new RepositoryError('NOT_FOUND'); const updated = current.suggestions.map((s) => s.id === id ? { ...s, status: 'rejected' as const } : s); await persist(user, current.consent, current.metrics, updated); return updated.find((s) => s.id === id)!; },
    async reset(user) { const current = await read(user); await persist(user, false, { sampleSize: 0, byCategory: {}, byEnergy: {}, preferredWindows: [], rescheduleRate: 0 }, []); },
    async accepted(user) { const current = await read(user); return current.consent ? current.suggestions.filter((s) => s.status === 'accepted') : []; },
    async schedulerAdjustments(user) { return (await this.accepted(user)).map((s) => ({ ...(typeof s.adjustment.category === 'string' ? { category: s.adjustment.category } : {}), ...(s.adjustment.energy === 'low' || s.adjustment.energy === 'medium' || s.adjustment.energy === 'high' ? { energy: s.adjustment.energy } : {}), multiplier: clamp(Number(s.adjustment.multiplier), .75, 1.75) })); },
  };
}
