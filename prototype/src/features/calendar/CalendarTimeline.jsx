...
export function CalendarTimeline({ dragMode = false, onSelect, items: providedItems }) {
  ...
  return (
    <div className={`calendar-timeline ${dragMode ? "is-dragging" : ""}`}>
      ...
      <div className="calendar-grid">
        ...
        {items.map((item) => {
          const placement = placements[item.id] ?? { top: Math.max(4, (Number(item.start?.slice?.(0, 2)) - 9) * 55), height: 42 };
