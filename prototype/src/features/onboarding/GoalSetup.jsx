...
export function GoalSetup({ screenId, onBack, onNext, onRoute, apiClient, demoMode = false }) {
  ...
  return <AppFrame title="Головна мета" eyebrow="Крок 3 з 4" onBack={onBack} noNav footer={footerByScreen[screenId]} footerRows={twoRows ? 2 : 1} contentAlign={centered ? "center" : "start"}>
    ...
    {screenId === "goal-test-question" ? <section className="goal-test-result-content"><div className="section-heading"><span className="success-chip">{session?.answers?.length ?? 0} з {session?.questions?.length ?? 3}</span><h1>{question?.prompt ?? "Що зараз для тебе найважливіше?"}</h1><p>Відповідай своїми словами. Це не тест і не діагностика.</p></div><VoiceTextComposer onTranscribe={transcribeGoalAnswer} onSubmit={submitAnswer} />{error ? <p role="alert" className="error-text">{error}</p> : null}</section> : null}
