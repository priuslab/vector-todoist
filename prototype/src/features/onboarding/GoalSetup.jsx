...
export function GoalSetup({ screenId, onBack, onNext, onRoute, apiClient, demoMode = false }) {
  ...
  const [manualTitle, setManualTitle] = useState(() => demoMode ? DEMO_GOAL.title : "");
  const [manualDescription, setManualDescription] = useState(() => demoMode ? "Хочу створити корисний проєкт і перевірити формат" : "");
  const [deadline, setDeadline] = useState(() => demoMode ? "2026-09-30" : "");
  const saveGoal = async (input) => {
    const { title, description, deadline: goalDeadline } = input ?? {};
    const cleanTitle = title?.trim();
    if (!cleanTitle) {
      setError("Сформулюй мету, щоб продовжити.");
      return;
    }
    ...
    try {
      await apiClient.request("/api/v1/goals", {
        ...
        body: JSON.stringify({
          ...
          ...(goalDeadline ? { deadline: `${goalDeadline}T00:00:00+00:00` } : {}),
        }),
      });
      onNext();
    } catch {
      setError("Не вдалося зберегти мету. Спробуй ще раз.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const suggestion = session?.suggestion ?? (demoMode ? { title: DEMO_GOAL.title, rationale: "Рекомендація на основі твоїх пріоритетів. Її можна змінити.", confidence: 0.92, safetyNotice: "Це рекомендація для планування, а не медична чи психологічна оцінка.", editable: true } : null);
  const saveEditedSuggestion = async () => { const title = editedTitle.trim(); if (!title || !suggestion) return; try { const result = apiClient && session?.id !== "demo" ? await apiClient.request(`/api/v1/goals/discovery/sessions/${session.id}/suggestion`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ title, rationale: suggestion.rationale }) }) : { ...session, suggestion: { ...suggestion, title } }; if (result) setSession(result); setEditing(false); } catch { setError("Не вдалося зберегти зміни. Спробуй ще раз."); } };
  const question = session?.questions?.find((item) => !session.answers?.some((saved) => saved.id === item.id)) ?? session?.questions?.[0];
  const footerByScreen = {
    "goal-manual": <Button onClick={() => saveGoal({ title: manualTitle, description: manualDescription, deadline })} disabled={loading}>{loading ? "Зберігаю…" : "Зберегти мету"}</Button>,
    "goal-test-start": <Button onClick={start} disabled={loading}>{loading ? "Готую діалог…" : "Почати короткий діалог"}</Button>,
    "goal-test-result": <><Button onClick={() => saveGoal(suggestion)} disabled={loading}>{loading ? "Зберігаю…" : "Підтвердити мету"}</Button><button className="text-action" onClick={() => setEditing(true)}>Відредагувати</button></>,
  ...
  return <AppFrame title="Головна мета" eyebrow="Крок 3 з 4" onBack={onBack} noNav footer={footerByScreen[screenId]} footerRows={twoRows ? 2 : 1} contentAlign={centered ? "center" : "start"}>
    ...
    {screenId === "goal-manual" ? <><div className="section-heading"><h1>Сформулюй свою мету</h1><p>Можна неідеально — Вектор допоможе зробити її конкретнішою.</p></div><div className="form-stack"><label>Головна мета<textarea value={manualTitle} onChange={(event) => setManualTitle(event.target.value)} /></label><label>Чому це важливо?<textarea value={manualDescription} onChange={(event) => setManualDescription(event.target.value)} /></label><label htmlFor="goal-deadline">Строк<input id="goal-deadline" type="date" value={deadline} onChange={(event) => setDeadline(event.target.value)} /></label></div></> : null}
