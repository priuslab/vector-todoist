...
export function SettingsScreens({ screenId = "settings-home", onNavigate = () => {}, apiClient }) {
  ...
  if (screenId === "settings-adaptation") {
    const adaptationFooter = <><Button onClick={() => setSaved(true)}>Прийняти зміну</Button><Button variant="secondary" onClick={() => setSaved(true)}>Залишити як є</Button></>;
    return <AppFrame title="AI-адаптація" onBack={() => onNavigate("settings-home")} noNav footer={adaptationFooter} footerRows={2} contentAlign="center"><section className="adaptation"><span><Sparkle size={32} weight="duotone" /></span><h1>Вектор помітив закономірність</h1><p>Складні задачі сьогодні зайняли трохи більше часу. Це лише рекомендація — профіль не змінюється без твоєї згоди.</p><div className="adaptation-change"><span><small>Було</small><strong>Фокус-блок · 50 хв</strong></span><span><small>Рекомендація</small><strong>Фокус-блок · 60 хв</strong></span></div><InlineInsight>Ти можеш прийняти, відхилити або скинути цю рекомендацію у будь-який момент.</InlineInsight></section>{saved ? <UndoSnackbar message="Рішення збережено" onUndo={() => setSaved(false)} /> : null}</AppFrame>;
  }
