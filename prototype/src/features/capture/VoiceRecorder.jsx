...
export function VoiceRecorder({ transcript, onCancel, onFinish, demo = false }) {
  ...
  const elapsed = `${String(Math.floor(recorder.elapsedSeconds / 60)).padStart(2, "0")}:${String(recorder.elapsedSeconds % 60).padStart(2, "0")}`;
  ...
  return (
    <section className="recorder">
      <div className="recorder__status"><span className="record-dot" />{recorder.status === "paused" ? "На паузі" : recorder.status === "uploading" ? "Завершую запис…" : "Записую"} · {elapsed}</div>
