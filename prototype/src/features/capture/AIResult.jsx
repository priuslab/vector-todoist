...
export function AIResult({ onViewDay, onApply, onUndo, analysis, preview, applying, previewLoading, error }) {
  if (analysis) {
    const hasUnscheduledTasks = Boolean(preview?.unscheduledTaskIds?.length);
    return <section className="ai-result"><div className="result-hero"><CheckCircle size={38} weight="duotone" /><span><h1>{preview ? "План готовий до застосування" : "Аналіз готовий"}</h1><p>{analysis.summary}</p></span></div><p className="soft-copy"><strong>Рекомендація AI:</strong> впевненість {Math.round(analysis.confidence * 100)}%. {preview ? "Перевір план і застосуй його, коли будеш готовий." : "Перевір пропозиції перед додаванням у план."}</p><div className="result-summary">{(preview?.tasks ?? analysis.tasks).slice(0, 3).map((task) => <div key={task.id ?? task.title}><ListChecks size={21} /><span><strong>{task.title}</strong><small>{task.estimatedMinutes} хв · {task.priority}{task.status === "inbox" ? " · Inbox" : ""}</small></span></div>)}{(preview?.ideas ?? analysis.ideas).slice(0, 3).map((idea) => <div key={idea.id ?? idea.text}><Lightbulb size={21} /><span><strong>Ідея</strong><small>{idea.summary}</small></span></div>)}</div>{error ? <p role="alert" className="soft-copy">{error}</p> : null}<div className="scheduled-preview"><strong>{preview ? "План на сьогодні" : "Контекст"}</strong>{(preview?.warnings ?? analysis.context).slice(0, 3).map((item) => <span key={typeof item === "string" ? item : item.message}>{typeof item === "string" ? item : item.message}</span>)}</div>{preview ? <Button icon={ArrowRight} loading={applying} onClick={onApply}>{hasUnscheduledTasks ? "Зберегти в Inbox" : "Застосувати план"}</Button> : <Button icon={ArrowRight} loading={previewLoading} onClick={onViewDay}>Переглянути пропозиції</Button>}</section>;
  }
  return (
