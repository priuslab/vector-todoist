...
const authStateValue = authState ? readFileSync(authState, 'utf8') : null;
...
test.describe('Vector security, failure and accessibility audit', () => {
  ...
  for (const size of sizes) {
    test(`mobile ${size.width}x${size.height} has no clipping or missing navigation names`, async ({ page }) => {
      await page.addInitScript((raw) => window.localStorage.setItem('pocketbase_auth', raw), authStateValue);
